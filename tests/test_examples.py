...
def test_answer_confidence_example_runs_without_live_provider() -> None:
    result = subprocess.run(
        [sys.executable, str(ROOT / "examples" / "answer_confidence.py")],
        cwd=ROOT,
        capture_output=True,
        text=True,
        timeout=10,
    ...
    assert result.returncode == 0, result.stderr
    assert "Answer Confidence rerank example" in result.stdout
    assert "rank=01 id=vitamin_c answer_confidence=0.90" in result.stdout
    assert "rank=02 id=vitamin_b12 answer_confidence=0.80" in result.stdout
    assert "answer_calls=4" in result.stdout
