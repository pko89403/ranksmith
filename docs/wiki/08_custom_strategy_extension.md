...
새 Strategy 클래스를 만든다. 이렇게 하면 비교 단위, provider 계약, 오류 정책을
섞지 않고 유지할 수 있다.

## 언제 새 Strategy를 만드는가
- 기존 listwise / pairwise 알고리즘과 순위 생성 절차가 다를 때.
- provider를 쓰지 않는 deterministic reranking을 만들 때.
- 자체 scoring, rule, hybrid ranking, tournament, confidence aggregation을 실험할 때.
- provider 응답 계약이 기존 `ranking` 또는 `winner`와 다를 때.
...
## Provider를 쓰지 않는 Strategy
provider가 필요 없으면 `provider: object`로 받고 사용하지 않는다.

```python
class LengthStrategy:
    def rerank(self, *, query, documents, provider, top_k=None):
        del query, provider
...
class ProviderBackedStrategy:
    def rerank(self, *, query, documents, provider: LLMProvider, top_k=None):
        try:
...
- custom Strategy 내부 provider 실패는 Strategy가 직접 `RerankProviderError`로 분류해야 한다.
...
provider를 await해야 한다면 async provider protocol을 사용한다.
